...
unsigned int gmath(unsigned char g);
void i2c_mems_block_read(unsigned char *mems_ptr);
void i2c_mems_init(unsigned char mode);
void clear_mems_sensor_latch(void);
void reset_to_current_position(void);
...
void i2c_mems_init(unsigned char mode){
	...
	switch(mode){
		...
		case WAKE_UP:
					...
					i2c_mems_byte_write(FF_WU_THS_2,WU_THRESHOLD);	// Set wake-up threshold (thr*18) mg
...
unsigned int gmath(unsigned char g){	// 2's complement conversion
	if(g & 0x80)
		g = 57 - (255-g + 1);
	else
		g += 57;
